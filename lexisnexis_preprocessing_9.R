...
# 
# ############### NEW: Doing it with Splitted Data
# # Next, we carry out some data cleaning ####
setwd("/Users/giacomoraederscheidt/Dropbox/Paper_Just_Transitions/0 Data/LexisNexis/Data_Cleaned")
load("All_merged_cleaned_test3.RData") # !!!!!!!!!!!!!




splitted_data <- read_excel("/Users/giacomoraederscheidt/Dropbox/Paper_Just_Transitions/B Topic Modelling/Article_Snippets_for_STM.xlsx")
splitted_data <- splitted_data %>% 
  select(Article_Snippet_Transition, ID, Count_Transition)

# Merge the splitted article text to the original articles_df, using ID as common identifier
articles_df <- articles_df %>%
  left_join(splitted_data, by = "ID")
...
docs <- Corpus(VectorSource(articles_df$Article_Snippet_Transition)) #### DIFFERENT TO BEFORE!! Now use just the snippets not $Article
...
dfm <- dfm_remove(dfm, c("someth", "got", "perhap", "thing", "alway", "think",
                         ...
                         "stay", "yes", "ago", "per", "date", "say", "year")) ## Added after inspecting the frex-list 
...
save(docs, meta_df = meta_df, articles_df = articles_df, dfm = dfm, file = "All_merged_cleaned_splitted_new.RData")
